...
LunaFrameは、ESP32-C3 マイクロコントローラと円形ディスプレイGC9A01を搭載したバッジ型デバイスです。
Wi-Fi APモードで動作し、表示画像をスマートフォンなどから簡単に更新できます。

このディレクトリは、LunaFrameのファームウェアソースコードを格納しています。
...
### 2. 画像の更新
1. LunaFrameデバイスの電源を入れ、Wi-Fi設定画面から `LunaFrame` ネットワークに接続します。パスワードは不要です。
